...
and never reached the selector.

Measured with no model and no network (`measure_depth.py`): the real resolver was run up to the
...
Slovenia's New Delhi page says what it is in its first line, and states "Citizens of Bangladesh …
Visa is required" about 8,000 characters in. The selector was shown the first 2,000.

The code had argued, in a comment, that a page whose head does not say what it is will not be saved
by a window further in. That is true, and beside the point for this page. Replayed on one captured
packet, five selection calls each:
- **Head only:** picked 2 of 5.
- **Head plus up to 600 characters around the traveller's own country:** 5 of 5, for 0.8% more input.

India's packet was the regression check. Its decision page was picked 5 of 5 either way. The input
grew 4.4%, and the selector picked 16.4 pages against 13.2.

Shipped (`SELECTION_ANCHOR_CHARACTERS`). Three live runs: all "visa required" from the New Delhi
page, against 5 of 8 before.

### Credited, then lost: the plan call's input guard

The plan call refused any input over 80,000 characters. `india/BD/SA` had its answer credited and
the plan refused in 3 of 8 runs; the refused run's input was 91,787. Across 131 logged plan calls
the median input is 17,918 and the largest 73,630, so the guard sat just above normal traffic.
Replayed on the 91,787 input, the plan answered "visa required" 3 of 3, in 17–22s. The guard is now
160,000; only calls that refused change.

### Credited, then lost: a step linking a page it did not cite

The sentinel `germany/IN/GB` was re-run three times on the new code. Two gave the confirmed answer:
verified, 12 documents, the UK missions' checklist. The third was refused by entry 197's step link,
...
Replayed on two captured UAE plan inputs, five calls each (Personas errors not counted):

| input | current prompt | with 8g |
| --- | --- | --- |
| from a run that answered null | visa required 1 of 5 | 4 of 4 |
| from a run that answered | 2 of 2 | 5 of 5 |
