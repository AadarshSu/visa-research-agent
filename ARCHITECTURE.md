...
| Plan-call usage log | UTC day | appended, one line per plan call | **No** — deleting it costs a question | `var/usage/`, `research/model_usage.py` |
...
names a different winner. Each model call also records its prompt and packet characters (`RecallRecord.model_calls`), and **input size explains almost none of the time**: r=+0.33 and +0.48, against a 40% run-to-run swing on the same corridor (entry 144). It also records what the provider billed — input, output, cached, cache-write and reasoning tokens — from which a corridor costs **$0.28**: selection 59%, search 27%, roles 14%, with input **96%** of the model bill (entry 145). Cache writes, which OpenAI bills at 1.25× on GPT-5.6 and later, were added in entry 165, so the $0.28 was priced without them. The call that writes the plan is not part of a resolution — it runs on every web request, stored corridors included — so it is not in this record; it appends to `var/usage/plan-calls-YYYY-MM-DD.jsonl` instead (entry 165). An empty map means the
