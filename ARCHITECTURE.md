...
names a different winner. Each model call also records its prompt and packet characters (`RecallRecord.model_calls`), and **input size explains almost none of the time**: r=+0.33 and +0.48, against a 40% run-to-run swing on the same corridor (entry 144). It also records what the provider billed — input, output, cached and reasoning tokens — from which a corridor costs **$0.28**: selection 59%, search 27%, roles 14%, with input **96%** of the model bill (entry 145). An empty map means the
