"""Entry 214: the owner's inspection page for item 63's second round.

usage, from the repository root:
    .venv/bin/python var/item70-2026-09-24/review/build_review_data.py > data.json
Reads the two rounds' saved runs (`item63/` before today's fixes, `item63-round2/` after) and
writes what each plan told the traveller, with every official page it rests on, as JSON.
"""
...
ROUND_ONE = HERE / "item63"
ROUND_TWO = HERE / "item63-round2"
...
def detail(run_dir: Path) -> dict:
    ...
    out.update(
        ...
        refused=[
            ...
            for failure in plan["unavailable_sources"]
            if not failure["source_id"].startswith("checklist_unread_")
        ],
        where_to_apply=(
            {
